...
// macOS için müzik çalma fonksiyonu
void muzikCal() {    
    // Masaüstündeki müzik dosyasının tam yolu
    string musicPath = "/Users/" + string(getenv("USER")) + "/Desktop/doors-elevator-music (1).mp3";
    string testCommand = "test -f \"" + musicPath + "\"";
    
    if (system(testCommand.c_str()) == 0) {
        string playCommand = "afplay \"" + musicPath + "\" &";
        system(playCommand.c_str());
        
        // 28 saniye bekle (müziğin çalması için)
        sleep(31);
